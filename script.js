  ...
  fetch(url) 
    ...
    .then((data) => {
      ...
      <div class="weather__summary">
        <p><i class="wi ${icons[data.weather[0].icon]} weather-icon"></i> <span class="weather__celcius-value">${Math.floor(data.main.temp)}°C</span></p>
    ...
    .catch((error) => {
      const markup = `<h1 class="location">Error: city not found</h1>
      <div class="weather__summary">
      <p><i class="wi ${icons['09n']} forecast__icon"></i></p>
      </div>`;
      removeChildren(weather);
      weather.insertAdjacentHTML('beforeend', markup);
    })
...
geolocateIcons.forEach((icon) => {
  icon.addEventListener('click', (e) => {
    if (navigator.geolocation) {
      ...
      alert('Your browser does not suppor geolocation');
    }
  });
});